...
{
    float l = len2(a);
    return l > 1e-6f ? a * (1.f / l) : make_float2(0.f, 0.f);
}
    ...
    const float *sActive)
{
    ...
    {
        int ny = refCY + dy;
        if (ny < 0 || ny >= p.gridHeight)
            continue;
        for (int dx = -1; dx <= 1; dx++)
        {
            int nx = refCX + dx;
            if (nx < 0 || nx >= p.gridWidth)
                continue;
                            ...
                            const SimParams p)
{
    ...
    if (gid >= p.agentCount)
        return;
    if (p.evacuationMode && active[gid] < 0.5f)
        return;
...
)
{
    ...
    for (int c = 0; c < NC; c++)
    {
        ...
        for (const auto &ob : obs)
        {
            ...
            if (dx * dx + dy * dy < BLOCK_R * BLOCK_R)
            {
                markBlocked(c);
                break;
            }
                      ...
                      const std::vector<FlowVec> &ff)
{
    ...
    alloc((void **)&s->posX, fN);
    alloc((void **)&s->posY, fN);
    alloc((void **)&s->velX, fN);
    alloc((void **)&s->velY, fN);
    alloc((void **)&s->maxSpd, fN);
    alloc((void **)&s->active, fN);
    alloc((void **)&s->exitTime, fN);
    alloc((void **)&s->sPosX, fN);
    alloc((void **)&s->sPosY, fN);
    alloc((void **)&s->sVelX, fN);
    alloc((void **)&s->sVelY, fN);
    alloc((void **)&s->sMaxSpd, fN);
    alloc((void **)&s->sActive, fN);
    alloc((void **)&s->forceX, fN);
    alloc((void **)&s->forceY, fN);
    alloc((void **)&s->targetX, fN);
    alloc((void **)&s->targetY, fN);
    ...
    CK(cudaMemset(s->velX, 0, fN));
    CK(cudaMemset(s->velY, 0, fN));
    CK(cudaMemset(s->forceX, 0, fN));
    CK(cudaMemset(s->forceY, 0, fN));
                            ...
                            const std::vector<FlowVec> &ff)
{
    ...
    memset(s->velX, 0, fN);
    memset(s->velY, 0, fN);
    memset(s->forceX, 0, fN);
    memset(s->forceY, 0, fN);
    memset(s->exitTime, 0, fN);
    for (int i = 0; i < N; i++)
    {
        s->active[i] = 1.f;
        s->maxSpd[i] = MAX_SPEED;
    }
    ...
    for (int i = 0; i < FLOW_CELLS; i++)
        s->flowField[i] = make_float2(ff[i].x, ff[i].y);
                          ...
                          const SimParams &p)
{
    for (int i = 0; i < p.agentCount; i++)
    {
        if (p.evacuationMode && active[i] < 0.5f)
            continue;
